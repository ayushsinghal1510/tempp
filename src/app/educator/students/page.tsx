...
export default async function EducatorStudentsPage() {
  ...
  const rows = students
    .map((s) => ({
      ...s,
      stats: aggregate(s.rounds),
      stuckCount: stuckTopics(s.rounds).length,
    }))
    .sort((a, b) => a.name.localeCompare(b.name));

  return (
    <DashboardShell user={user} nav={EDUCATOR_NAV} title="Students">
      <div className="space-y-6">
        <section className="card p-6">
          ...
          <p className="mt-0.5 text-sm text-muted">
            Sessions a student ran on companies they added themselves are not
            shown here — those are private to them.
          </p>
        </section>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.userId} className="border-t border-line">
                    ...
                    <td className="px-4 py-2.5">
                      {topicLabel(r.stats.bestTopic)}
                    </td>
                    <td className="px-4 py-2.5">
                      {topicLabel(r.stats.worstTopic)}
                    </td>
                    <td className="px-4 py-2.5 tabular-nums">
