...
export default function InterviewRoom({
  ...
  // Whether the `pr` score + props rail is on screen. Read twice — once to
  // render it, once to reserve the room it needs on the stage — so it is
  // computed once here rather than being two expressions that can drift.
  const prRailVisible =
    roleplay === "pr" && Boolean(parsedRunningScore || visibleProps.length > 0);
