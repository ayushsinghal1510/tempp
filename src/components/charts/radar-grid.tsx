...
export interface RadarGridProps {
  ...
  /**
   * Real-world value each ring represents 100% of (e.g. the true axis max
   * when a series has been normalized 0-100 against a non-100 scale).
   * When set, level labels show `(level/levels) * maxValue` instead of the
      ...
      {showLabels &&
        [...new Array(levels)].map((_, i) => (
          ...
          >
            ...
            >
              {maxValue != null
                ? Number((((i + 1) / levels) * maxValue).toFixed(1)).toString()
                : ((i + 1) * 100) / levels}
            </text>
          </motion.g>
        ))}
