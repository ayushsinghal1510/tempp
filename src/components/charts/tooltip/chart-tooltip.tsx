...
const ChartTooltipInner = memo(function ChartTooltipInner({
  ...
  const title = useMemo(() => {
    ...
    // A row's own `label` (e.g. "S1", elapsed mm:ss) wins over the formatted
    // date when present — same override the x-axis ticks already honor.
    return (
      dateLabels[tooltipData.index] ??
      weekdayDateFmt.format(xAccessor(tooltipData.point))
    );
  }, [tooltipData, barXAccessor, xAccessor, dateLabels]);
