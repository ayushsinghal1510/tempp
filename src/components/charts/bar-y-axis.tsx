...
export interface BarYAxisProps {
  ...
  maxLabels?: number;
  /** Optional second line under each label, keyed by that row's label string. */
...
function BarYAxisLabel({
  ...
  return (
    <div
      className="absolute right-0 flex flex-col items-end justify-center pr-2"
      style={{
        top: y,
        height: bandHeight,
      }}
    >
      ...
      {sublabel && (
        <span
          className="truncate whitespace-nowrap text-right text-[10px] text-faint"
          style={{ maxWidth: 70 }}
        >
          {sublabel}
        </span>
