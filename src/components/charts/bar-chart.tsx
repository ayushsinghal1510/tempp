...
export interface BarChartProps {
  ...
  /** Force the value-axis max instead of auto-fitting to the data's own range. */
  yDomainMax?: number;
...
const ChartCore = memo(function ChartCore({
  ...
  const maxValue = useMemo(() => {
    ...
    return max || 100;
  }, [data, lines, stacked, yDomainMax]);

  // Value scale (linear) - for the value axis. A caller-supplied yDomainMax
  // is an already-known exact bound (e.g. "out of 10"), so skip the 1.1
  // headroom and `nice` rounding used for the auto-fit case.
  const valueScale = useMemo(() => {
    const range = isHorizontal ? [0, innerWidth] : [innerHeight, 0];
    return yDomainMax != null
      ? scaleLinear({ range, domain: [0, maxValue] })
      : scaleLinear({ range, domain: [0, maxValue * 1.1], nice: true });
  }, [innerWidth, innerHeight, maxValue, isHorizontal, yDomainMax]);
  ...
  }, [data, innerHeight, isHorizontal, lines, valueScale, yDomainMax]);
