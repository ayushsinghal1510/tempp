...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  // A row's own `label` (e.g. "S1") wins over the formatted date when present,
  // so category-like time series (sessions, turns) can show their natural
  // label on the x-axis instead of the synthetic date used to position them.
  const dateLabels = useMemo(
    () =>
      visiblePlotData.map((d) =>
        typeof d.label === "string" ? d.label : shortDateFmt.format(xAccessor(d))
      ),
